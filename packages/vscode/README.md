...
- **Three sidebar sections** — **Profiles**, **Companies**, and **Endpoint Browser** (API routes and published **ODataV4** services → entity sets), all reading from on-disk caches. Click a profile to make it active; the Companies and Endpoints sections follow it (the active profile shows in their headers). Click a company to make it the default (★ marks the current one). Browsing needs no credentials; only *Discover*/*Refresh* talk to BC. The VS Code records experience is intentionally read-only; CLI and MCP support writes to APIs and writable ODataV4 pages.
...
- **Search Endpoints** — the magnifier in the Endpoint Browser toolbar opens a search box over every cached entity set. Type part of a name, a route, or an entity type (`Microsoft.NAV.customer`) and press Enter to open the records grid — no expanding routes to find the one you want. The search itself reads the on-disk cache and needs no connection; opening the grid then fetches the first page, like clicking the entity set in the tree.
...
This extension is one of five faces of navapi that share the same profiles and secrets (in `~/.navapi`) — so anything you set up here works everywhere:
...
- **`navapi ui`** — a secure local browser interface for profiles, companies, endpoint discovery, schemas, and read-only record queries.
