...
**Marketplace vsixes:** `pnpm --filter navapi-vscode package:all` (`scripts/package-all.mjs`) builds one vsix per VS Code target under `dist/vsix/`, each bundling only that platform's `@napi-rs/keyring` binding (downloaded from npm via `npm install --prefix`, not the flaky host `tar`). The plain `package` script is still host-binding-only for local dev. Gotchas baked in: Marketplace rejects SVG as the extension `icon` (use `media/icon.png`, regenerated by `scripts/make-icon.mjs` from the compass SVG — no image tooling needed); `CHANGELOG.md` must be `!`-allowlisted in `.vscodeignore` or it won't ship; vsce lowercases readme/changelog/license inside the vsix. Publisher `jeremy-vyska` already exists on the Marketplace; `vsce publish` needs the user's PAT.
