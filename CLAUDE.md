...
- **Two auth strategies, one branch point.** `ProfileConfig.authType` is `'clientCredentials'` (the default when the field is absent, so old `profiles.json` files keep working) or `'azureCli'`. `createClientForProfile()` in `factory.ts` is the only place that branches — that's why az-cli auth works in the CLI, MCP, and VS Code with no changes to any of them. The az-cli path must skip secret resolution entirely, or it fails on a missing secret that was never needed. `AzureCliAuth` shells out to `az account get-access-token -o json` (`-o tsv` has no expiry, which would cost a subprocess per request); it takes an injectable `exec` so tests never spawn anything, and validates tenant/resource against a strict charset because Windows needs `shell: true` to run `az.cmd`.
- **az picks the identity from its active account, not from `--tenant`.** Cross-tenant works whenever an identity az holds has access (GDAP, guest, or a native account) — otherwise you get AADSTS50020, which means "wrong identity", not "wrong tenant". `ProfileConfig.azAccount` pins one: navapi resolves the username against `az account list --all`, requires the match to be **in the profile's tenant** (otherwise you'd silently get another tenant's token), and passes `--subscription <id>`. `--subscription` and `--tenant` are mutually exclusive in az, and `--subscription` does *not* change az's global active account. The id passed on the command line always comes from az's own output, never from user input.
